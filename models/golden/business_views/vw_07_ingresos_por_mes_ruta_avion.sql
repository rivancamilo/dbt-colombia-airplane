-- Pregunta de negocio: ¿Cuál es el ingreso total (valor final tras descuentos) por mes, por ruta y por avión?
select
    fd.anio_mes,
    o.ciudad         as ciudad_origen,
    d.ciudad         as ciudad_destino,
    av.nombre_avion,
    round(sum(b.valor_final), 2) as ingresos_totales,
    count(*)                     as cantidad_billetes
from {{ ref('fact_billete') }} b
join {{ ref('fact_vuelo') }} v on v.id_vuelo = b.id_vuelo
join {{ ref('dim_fecha') }} fd on fd.date_day = b.fecha_salida
join {{ ref('dim_aeropuerto') }} o on o.id_aeropuerto = v.id_aeropuerto_origen
join {{ ref('dim_aeropuerto') }} d on d.id_aeropuerto = v.id_aeropuerto_destino
join {{ ref('dim_avion') }} av on av.id_avion = v.id_avion
where b.tipo_billete = 'NORMAL'
group by 1, 2, 3, 4
order by anio_mes, ingresos_totales desc
